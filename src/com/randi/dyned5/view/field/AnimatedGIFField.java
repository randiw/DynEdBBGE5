package com.randi.dyned5.view.field;
...
public class AnimatedGIFField extends BitmapField {
	...
	protected void paint(Graphics graphics) {
		...
		if (_currentFrame != 0) {
			...
			graphics.drawImage(image.getFrameLeft(_currentFrame), image.getFrameTop(_currentFrame), image.getFrameWidth(_currentFrame), image
					.getFrameHeight(_currentFrame), image, _currentFrame, 0, 0);
		}
	}
	...
	private class AnimatorThread extends Thread {
		...
		public AnimatorThread(AnimatedGIFField theField) {
			_theField = theField;
			_totalFrames = image.getFrameCount();
			_totalLoops = image.getIterations();
		...
		public void run() {
			while (_keepGoing) {
				...
				try {
					...
					// is drawn.
					sleep(image.getFrameDelay(_currentFrame) * 10);
